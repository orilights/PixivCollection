<template>
  <li class="absolute wf-image bg-gray-100 rounded-[12px] overflow-hidden transition-[top,left] group">
    <div
    ...
    >
      ...
      <p>标题：{{ image.title }}</p>
      <p>作者：{{ image.detail.author.name }}</p>
      <p>
        标签：<span
    ...
    <img v-if="loadImage" class="w-full" :src="image.preview" @load="$emit('imageLoaded', index)">
  </li>
</template>
...
defineEmits(['openImage', 'openPixiv', 'imageLoaded'])
</script>
